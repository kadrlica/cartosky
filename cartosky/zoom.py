...
from cartosky.core import OrthoSkymap
...
class SurveyZoom(SurveyMcBryde):
    ...
    def draw_parallels(*args, **kwargs):
        return

    def draw_meridians(*args, **kwargs):
        return
    ...
    def set_axes_limits(self):
        ...
        # Should this be extent instead of self.EXTENT?
...
class DESSkymapQ1(DESSkymapMcBryde):
    ...
    def draw_inset_colorbar(self, *args, **kwargs):
        defaults = dict(loc=4, height="6%", width="20%",
...
class DESSkymapQ2(DESSkymapMcBryde):
    ...
    def draw_inset_colorbar(self, *args, **kwargs):
        defaults = dict(loc=2, width="30%", height="4%",
...
class DESSkymapQ4(DESSkymapMcBryde):
    ...
    def draw_inset_colorbar(self, *args, **kwargs):
        defaults = dict(loc=3, width="30%", height="4%",
                        bbox_to_anchor=(0, 0.05, 1, 1))
        setdefaults(kwargs, defaults)
        super(DESSkymapMcBryde, self).draw_inset_colorbar(*args, **kwargs)
...
class DESLambert(SurveySkymap):
    ...
    def draw_parallels(self, *args, **kwargs):
        ...
        ax = plt.gca()
        for k in ret.keys():
            ax.annotate(r"$%+d{}^{\circ}$"%(k), self(0, k), xycoords='data',
